-module(role_handler_tests).

-include_lib("eunit/include/eunit.hrl").
% Include etest's assertion macros.
% -include_lib("etest/include/etest.hrl").

-export([teardown/0, setup/0, role_handler_tests_/0]).
...
init_list_operation_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/roles/all">> end),

    %% Act
    {cowboy_rest, ReqOut, State} = role_handler:init(Req, #{}),
...
init_single_operation_with_id_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/roles/123">> end),
...
allowed_methods_list_operation_test() ->
    ...
    %% Act
    {[Methods], ReqOut, StateOut} = role_handler:allowed_methods(Req, State),
...
%     meck:expect(redis_handler, read_all, fun ("role") -> ExpectedResponse end),

%     % io:format("Formatted response: ~p~n", [ExpectedResponse]),
%     io:format("Actualresponse: ~p~n", [role_handler:handle_get_all(Req, State)]),
%     %% Act
%     {Response, _ReqOut, _StateOut} = role_handler:handle_get_all(Req, State),
...
%     meck:expect(redis_handler, read, fun ("role", _Id) -> ExpectedResponse end),

%     % Act
%     {Response, _ReqOut, _StateOut} = role_handler:handle_get_by_id(Id, Req, State),
...
%     meck:expect(redis_handler, read, fun ("role", _Id) -> undefined end),
%     meck:expect(cowboy_req, reply, fun (404, _, _, ReqIn) -> ReqIn end),

%     % Act
%     {Result, ReqOut, StateOut} = role_handler:handle_get_by_id(Id, Req, State),
...
delete_resource_test() ->
    ...
    meck:expect(redis_handler, delete, fun ("role", _Id) -> io:format("Id deleted: ~p~n", [Id]), ok end),

    %% Act
    {Response, ReqOut, StateOut} = role_handler:delete_resource(Req, State),
