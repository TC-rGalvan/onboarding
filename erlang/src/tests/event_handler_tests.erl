...
-export([teardown/0, setup/0, event_handler_tests_/0]).
...
init_list_operation_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/events/all">> end),

    %% Act
    {cowboy_rest, ReqOut, State} = event_handler:init(Req, #{}),
...
init_single_operation_with_id_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/events/123">> end),
...
allowed_methods_list_operation_test() ->
    ...
    %% Act
    {[Methods], ReqOut, StateOut} = event_handler:allowed_methods(Req, State),
...
delete_resource_test() ->
    ...
    meck:expect(redis_handler, delete, fun ("event", _Id) -> io:format("Id deleted: ~p~n", [Id]), ok end),

    %% Act
    {Response, ReqOut, StateOut} = event_handler:delete_resource(Req, State),
