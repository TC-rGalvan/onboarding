...
-export([teardown/0, setup/0, user_handler_tests_/0]).
...
init_list_operation_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/users/all">> end),

    %% Act
    {cowboy_rest, ReqOut, State} = user_handler:init(Req, #{}),
...
init_single_operation_with_id_test() ->
    ...
    meck:expect(cowboy_req, path, fun (_) -> <<"/users/123">> end),
...
allowed_methods_list_operation_test() ->
    ...
    %% Act
    {[Methods], ReqOut, StateOut} = user_handler:allowed_methods(Req, State),
...
handle_get_all_test() ->
    ...
    meck:expect(redis_handler, read_all, fun ("user") -> ExpectedResponse end),

    % io:format("Formatted response: ~p~n", [ExpectedResponse]),
    io:format("Actualresponse: ~p~n", [user_handler:handle_get_all(Req, State)]),
    %% Act
    {Response, _ReqOut, _StateOut} = user_handler:handle_get_all(Req, State),
...
handle_get_by_id_found_test() ->
    ...
    meck:expect(redis_handler, read, fun ("user", _Id) -> ExpectedResponse end),

    % Act
    {Response, _ReqOut, _StateOut} = user_handler:handle_get_by_id(Id, Req, State),
...
handle_get_by_id_not_found_test() ->
    ...
    meck:expect(redis_handler, read, fun ("user", _Id) -> undefined end),
    meck:expect(cowboy_req, reply, fun (404, _, _, ReqIn) -> ReqIn end),

    % Act
    {Result, ReqOut, StateOut} = user_handler:handle_get_by_id(Id, Req, State),
...
delete_resource_test() ->
    ...
    meck:expect(redis_handler, delete, fun ("user", _Id) -> io:format("Id deleted: ~p~n", [Id]), ok end),

    %% Act
    {Response, ReqOut, StateOut} = user_handler:delete_resource(Req, State),
